"""get_works_list.py, use IMSLP API to get a list of works using a page Id .
...
from rarc_utils.misc import chainGatherRes, unnest_assign_cols
...
def get_imslp(
    api_type=2, max_pages=None, id_remap_: Callable = id_remap
    ...
    while moreresultsavailable:
        ...
        catd = {
            d["id"]: d for k, d in jsond.items() if "id" in d
        }  # last metadata dict does not have 'id' key
        data |= catd
        ...
        logger.info(
            f"{len(catd)=:,} {len(data)=:,}, new {start=:<10,} {moreresultsavailable=}"
        )
...
def get_multi_zset(key: str) -> List[Dict[str, Any]]:
    """Get multiple items from redis sorted set.
...
def save_multi_zset(key: str, items: List[dict], mininterval=1) -> None:
    """Save multiple items to redis sorted set."""
...
re_finalize = {"size": float}  # lambda x: float(x), # less strict


def regex_parse_fields(text):
    """Parse fields from raw scrape result."""
    ...
    for patt, matcher in re_patterns.items():
        ...
        if len(matches) == 1:
            ...
            logger.warning(
                f"cannot parse '{patt}', {len(matches)=} {matches=} row.{text=}"
            )
        else:
            ...
            try:
                ...
                logger.warning(
                    f"cannot parse list of 'npage' items, {len(matches)=} {matches=}"
                )
...
def parse_metadata_table(text):
    """Parse metadata from html table.
...
async def extract_all_items(Id=None, data=None) -> dict:
    ...
    info_matches = [
        box.find(attrs={"class": "we_file_download plainlinks"}) for box in boxes
    ]
    ...
    for i, box in enumerate(boxes):
        ...
        if downloadDiv is None or "href" not in downloadDiv.attrs:
            logger.warning(
                f"downloadDiv None or attr 'href' not in downloadDiv, cannot extract downloadUrl. {Id=} {downloadDiv=}"
            )
        ...
        if len(findRix) == 1:
            ...
            d[i][
                "parent_meta"
            ] = metadata  # todo: should eventually become psql table, so that metadata gets saved only once for the parent
            ...
            if args.saveRaw:
                await rcon.aior[REDIS_DB].zadd(
                    redisKeyRawSoup, {json.dumps(str(soup)): rix}
                )  # rcon.r.zadd
...
def extract_dict_keys(row) -> List[str]:
    """Extract dictionary keys from row."""
...
def rdata_to_df(
    dcounts: Union[List[dict], Dict[str, Dict[int, dict]]],
    renameDict=None,
    sortBy=None,
    includeJson=True,
    unnestCols=False,
) -> Tuple[List[str], pd.DataFrame]:
    """Parse redis data to dataframe."""
    tuples, vals = [], []
    ...
    if unnestCols:
        ...
        ul = (
            [(renameDict["parent_meta"], i) for i in metaKeys]
            if "parent_meta" in renameDict
            else [("parent_meta", i) for i in metaKeys]
        )
...
async def fetch_all(session, urls: Dict[str, str]):
    """Fetch all scrape results from list of urls.
    ...
    for idx, url in enumerate(urls):
        print(
            "{}: {}".format(url, "ERR" if isinstance(results[idx], Exception) else "OK")
        )
...
class ArgParser:
    ...
    def populate_parser(cls):
        """Populate parser with command line arguments."""
        ...
        CLI.add_argument(
            "--nrow", type=str, default=None, help="number of rows to scrape"
        )
