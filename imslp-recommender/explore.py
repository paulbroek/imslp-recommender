"""explore.py, load scraped data from redis and apply some data science to it.
...
import pandas as pd
from get_works_list import rdata_to_df  # save_multi_zset,
from get_works_list import get_multi_zset, unique_nested_lists
...
metaCols, df = rdata_to_df(
    rdata, renameDict={"parent_meta": "meta"}, sortBy="ndownload", unnestCols=True
)  # scrapeDate
...
def hasCat(item, category: str | int) -> bool:
    """Determine if item has `category`."""
